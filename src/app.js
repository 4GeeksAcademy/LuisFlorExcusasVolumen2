...
window.onload = function() {
  const preguntas = [
    ...
    "¿El país dónde escondiste el diamante es conocido por su fabricacion de relojes?",
    ...
    "Pregunta libre! Recuerda: solo puedes hacer preguntas que se puedan responder con sí o no."
  ];
  function randomizar() {
    let resultados = [];
    for (let i = 0; i < 7; i++) {
      // 7 combinaciones
      let randomNumber1 = Math.floor(Math.random() * 131);
      let randomNumber2 = Math.floor(Math.random() * 131);
      resultados.push(
        `${preguntas[randomNumber1]} ${preguntas[randomNumber2]}`
      ); // Agrega cada combinación al array
    }
    return resultados; // Devuelve el array completo de combinaciones
  }

  function mostrarTexto() {
    let texto = randomizar().join("<br>"); // Llama a randomizar y convierte el array en una string con saltos de línea
    let elemento = document.getElementById("excusa");
    elemento.innerHTML = texto; // Muestra el resultado en el elemento HTML
  }
  mostrarTexto();
};
